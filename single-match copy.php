...
if (have_posts()) {
    while (have_posts()) {
        ...
        $video_data_urls = get_post_meta(get_the_ID(), 'video_data_urls', true);
        $video_data_urls = json_decode($video_data_urls, true);  // Convert the JSON string back to PHP array

        // Check if video_data_urls is set and is an array
        if (isset($video_data_urls) && is_array($video_data_urls)) {
            // Get the first iframe URL
            $video_iframe_url = $video_data_urls[0]['iframe'];
        }

        // var_dump($video_data_urls);
        ...
        // Retrieve commentator picture using get_post_meta
        $commentator_picture = get_post_meta(get_the_ID(), 'commentator_picture', true);
        ...
        <main class="main-layout">
            <section class="single-match-container">
                ...
                <div class="match-stream" id="match-stream" >
                    <iframe src="<?php echo esc_url($video_iframe_url); ?>" width="500" height="300"></iframe>
                </div>
            </section>
